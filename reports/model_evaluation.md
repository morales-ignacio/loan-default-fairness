...
This document consolidates the analytical work in `notebooks/06_random_forest.ipynb`, `notebooks/07_lightgbm.ipynb`, and `notebooks/08_calibration_thresholds.ipynb`.

## Summary

Built and evaluated eight model variants on the temporally split LendingClub dataset (train 2014-2015, val 2016, test 2017 held out). Champion model is a calibrated LightGBM tuned via Optuna. On the held-out half of val (val_eval), it achieves **AUC 0.730** and **Brier 0.1572** at an empirically optimal threshold of **0.1592**, very close to the theoretical break-even of 0.1667. On full val (2016), the underlying model scores AUC 0.7280 / Brier 0.1578.

Compared to the grade-only baseline (all on full val), the champion delivers:

- **+0.046 AUC** (0.682 to 0.728)
- **-5% Brier** (0.166 to 0.158), preserving the well-calibrated property
- **+$23M projected profit** on full val ($66M vs $43M)

The model adds genuine independent value: the int_rate ablation confirmed that performance does not depend on LendingClub's own risk scoring baked into the interest rate feature.

## Final scoreboard

All runs are logged to `reports/results.json` (appended in order by notebooks 05-07).
...
| logreg_no_int_rate | 0.715 | 0.425 | 0.216 | $59.9M | Ablation |
| rf_baseline | 0.716 | 0.427 | 0.201 | $60.5M | Tree baseline |
| rf_no_balance | 0.716 | 0.428 | 0.161 | $60.5M | Class-weight ablation |
| lgbm_baseline | 0.725 | 0.441 | 0.159 | $63.7M | Gradient boosting |
| lgbm_tuned | 0.728 | 0.445 | 0.158 | $65.4M | Optuna (50 trials) |
...
**Key finding: the signal in this dataset is mostly linear.** Random Forest matched logistic regression's AUC almost exactly (0.7159 vs 0.7157), indicating that the predictive patterns are well-approximated by a weighted sum of features. Non-linear interactions exist but are not the dominant source of predictive power.

LightGBM provided a modest but real improvement (+0.012 AUC over RF), justifying the architectural step up. The gain comes from sequential error correction: each tree explicitly targets borrowers the previous trees got wrong, capturing subtle credit patterns that parallel averaging cannot.

This finding informed the optimization strategy: extensive RF hyperparameter tuning would have low yield (no non-linear patterns to find), so optimization effort was concentrated on LightGBM.
...
|---|---|---|---|---|
| balanced | 0.7159 | 0.2005 | $60.5M | 0.3581 |
| None | 0.7157 | **0.1609 (-20%)** | $60.5M | **0.1409** |

**Interpretation**: class balancing shifts probability lecels, not ranking. AUC and profit (which depend only on ranking) are unchanged. Brier (which depends on probability calibration) improves dramatically.

The optimal threshold also drops from 0.36 to 0.14, because the model now outputs honest probabilities. With `class_weight='balanced'` inflating predictions, the cutoff sits artificially high; with raw probabilities, the cutoff approaches the theoretical break-even rate of 0.1667.
...
Used Optuna's TPE (Tree-structured Parzen Estimator) sampler for 50 trials over an 8-dimensional hyperparameter space. Each trial trained a LightGBM with early stopping on val AUC (50-round patience), with `n_estimators` capped at 1000. The sampler was seeded (`TPESampler(seed=42)`) so the search is reproducible run to run.

### Best parameters found

```
learning_rate:     0.0156    (default: 0.10)
num_leaves:        90        (default: 31)
min_child_samples: 60        (default: 20)
feature_fraction:  0.585     (default: 1.0)
bagging_fraction:  0.712     (default: 1.0)
bagging_freq:      3
lambda_l1:         0.0068
lambda_l2:         9.47      (near search boundary of 10)
```
...
- **Slow learning** (learning_rate 0.016, less than a sixth of the default) forces incremental updates
- **Strong leaf-level regularization** (min_child_samples 60, vs default 20) prevents overfitting to rare borrower patterns
- **Column and row subsampling** (feature_fraction 0.585, bagging_fraction 0.712) adds noise to each tree
- **Maximum L2 regularization** (lambda_l2 near the search ceiling) further constrains leaf weights
- **High num_leaves** (90 vs default 31) gives each tree more flexibility within the regularized constraints

This pattern is consistent with credit risk datasets: the underlying signal is small and stable, so the model wants to learn slowly with many constraints, but is allowed to explore complex per-tree structure when those constraints permit.

### Performance

- Search time: approximately 60 minutes for 50 trials
- Improvement over LightGBM baseline: AUC 0.7250 to 0.7280 (+0.0030)
- Actual iterations used: 1000 of 1000 max (vs 501 for baseline). The slower learning rate consumed the full budget.
...
Pre-calibration reliability diagrams (see `reports/figures/calibration_before.png`) show:
...
Post-calibration (see `reports/figures/calibration_comparison.png`), both curves align with the diagonal.
...
| LightGBM tuned | 0.1580 | 0.1572 | -0.5% |
...
The LogReg's 27% Brier reduction is dramatic. The LightGBM's 0.5% is essentially noise. The champion model didn't need calibration because the architecture decisions (gradient boosting, no `class_weight='balanced'`) already delivered well-calibrated probabilities.
...
The profit curve for the calibrated champion (see `reports/figures/profit_curve.png`) shows a clean inverted-U shape with a clear peak.

- Empirical optimum: threshold 0.1592, profit $33.3M on val_eval
- Theoretical break-even: 0.1667 (derived from cost matrix: gain_rate / (gain_rate + loss_rate))

The 0.008 gap between empirical and theoretical is small, reflecting how well-calibrated the model is. With a poorly calibrated model, this gap would exceed 0.10. The close agreement between theory and empirics validates the calibration work.

### Marginal vs average default rate

The approval rate vs default rate plot (see `reports/figures/approval_default_tradeoff.png`) reveals a subtle but important insight.

At the optimal approval rate (37.3%), the **average** default rate among approved loans is only 9.2%, well below the break-even rate of 16.7%. The optimum is not where average default rate equals break-even.
...
The cost matrix assumptions (gain_rate=0.10, loss_rate=0.50) drive profit projections substantially. A sensitivity sweep across plausible ranges (see `reports/figures/cost_sensitivity.png`) shows:

- Profit range: $2.4M (worst case: gain=0.04, loss=0.70) to $180.3M (best case: gain=0.20, loss=0.30)
- Threshold range: 0.045 to 0.376
...
| Metric | Value |
|---|---|
| AUC | 0.730 (val_eval); 0.7280 on full val |
| Brier | 0.1572 |
| Optimal threshold | 0.1592 |
| Approval rate at optimum | 37.3% |
| Default rate among approved | 9.2% |
...
```python
import joblib
artifact = joblib.load('models/champion_lgbm.pkl')
...
- **Test (2017, 169k loans)**: untouched throughout model development. Reserved for final unbiased evaluation after the fairness work.
...
All models share the same preprocessing pipeline (the `ColumnTransformer` built in notebooks 06-08):

- 79 features used (after dropping 6 redundant, ~17 sec_app/joint, 2 high-cardinality, 1 split column, from the 105-column clean parquet)
...
Run metrics are appended to `reports/results.json` as each modeling notebook runs (notebooks must be run in order, since each reads the previous one's saved results). The champion model artifact is generated by the final cell of notebook 08 and saved to `models/champion_lgbm.pkl`.